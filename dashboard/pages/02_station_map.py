...
def load_station_map_data() -> pd.DataFrame:
    ...
    def score_to_hex(score):
        ...
        elif score < 0.3: return "#EAB308"   # amber
        elif score < 0.5: return "#F97316"   # orange
...
if HAS_PYDECK and len(filtered) > 0:
    ...
    st.map(filtered[["latitude","longitude"]].rename(columns={"latitude":"lat","longitude":"lon"}))
else:
...
if not HAS_FOLIUM:
    ...
    # ── Layer: clustered markers (all filtered stations) ──────────
        ...
        <div style="font-family: Arial, sans-serif; min-width: 200px;">
            ...
            <table style="width:100%; font-size:12px; border-collapse:collapse;">
                ...
                <tr style="background:#F1F5F9;">
                    ...
                    <td style="padding:4px 6px;">{int(row.get('nbdocks', row.get('docks_count', 0)))}</td>
                </tr>
                <tr>
    ...
    # ── Layer: heatmap of imbalance intensity ─────────────────────
    ...
    # ── Layer: circle markers for CRITICAL stations only ──────────
    critical_group = folium.FeatureGroup(name="Critical Stations Only", show=False)
    critical = filtered[filtered["priority"] == "CRITICAL"]
    ...
    # ── Show clicked station details ──────────────────────────────
    if map_data and map_data.get("last_object_clicked_popup"):
        st.info("Click a marker on the map to see station details in the popup.")
...
# handle both mock (nbdocks) and live (docks_count) column names
