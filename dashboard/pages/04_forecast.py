...
def load_stations(use_mock=True):
    if use_mock:
        ...
        with engine.connect() as conn:
            return pd.read_sql(
                text(
                    f"SELECT station_id, station_name, nb_docks, latitude, longitude "
                    f"FROM `{PROJECT}.{DATASET}.dim_stations`"
                ),
                conn,
            )
...
with col_sel:
    selected_name = st.selectbox(
        "Select station", stations["station_name"].tolist(), index=0
    )
...
with col_info:
    ...
    **Location:** {row.get('latitude', row.get('lat', '?')):.4f}, {row.get('longitude', row.get('lon', '?')):.4f}
    ...
    if not MODEL_PATH.exists():
        st.info(
            "Trained model not found — showing synthetic pattern. "
...
st.caption(
    "Teal = forecast  |  Shaded = 95% confidence interval  |  "
...
st.dataframe(
    display[["Hour", "forecast", "lower_ci", "upper_ci", "Peak?"]].rename(
        columns={
